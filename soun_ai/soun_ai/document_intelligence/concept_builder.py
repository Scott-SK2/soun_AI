from __future__ import annotations

from typing import List, Dict, Tuple
import re

from document_intelligence.document_store import DocChunk

# -----------------------
# Heuristics / constants
# -----------------------

STOP_TITLES = {
    "introduction", "agenda", "overview", "summary", "conclusion",
    "learning objectives", "objectives", "table of contents",
    "course overview", "course material", "course materials",
}

BAD_TITLES = {
    "what", "why", "how", "when", "where", "who", "which",
    "there", "this", "that", "these", "those", "it", "we",
    "slide", "chapter", "case", "example",
    "intro", "intro…personal", "intro…academic", "intro…practice",
}

# -----------------------
# Helpers
# -----------------------

def _clean(s: str) -> str:
    s = (s or "").replace("\x00", " ")
    s = re.sub(r"\s+", " ", s)
    return s.strip()

def _slug(s: str) -> str:
    s = _clean(s).lower()
    s = re.sub(r"[^a-z0-9\s-]", "", s)
    s = re.sub(r"\s+", "-", s).strip("-")
    return s[:60] if len(s) > 60 else s

def _is_footerish(text: str) -> bool:
    t = (text or "")
    low = t.lower()
    return ("©" in t) or ("prof." in low) or ("faculty" in low) or ("copyright" in low)

def _strip_leading_numbering(s: str) -> str:
    s = _clean(s)
    # remove "Slide 3 - " or "3." or "12 1." etc.
    s = re.sub(r"^(slide\s*)?\d+(\.\d+)?\s*[-:.)]*\s*", "", s, flags=re.I)
    s = re.sub(r"^\d+\s+\d+(\.\d+)?\s*[-:.)]*\s*", "", s)
    return s.strip()

def _strip_trailing_slide_number(s: str) -> str:
    s = _clean(s)
    # remove trailing " 10", " 4", etc.
    s = re.sub(r"\s\d{1,2}$", "", s).strip()
    return s

def _is_question_title(s: str) -> bool:
    s = _clean(s)
    if "?" in s:
        return True
    # question words at beginning are often non-concepts
    return bool(re.match(r"^(what|why|how|when|where|who|which)\b", s.lower()))

def _normalize_title(s: str) -> str:
    s = _strip_leading_numbering(s)
    s = _strip_trailing_slide_number(s)
    s = _clean(s)
    return s

def _title_is_valid(title: str) -> bool:
    t = _clean(title)
    if not t:
        return False
    low = t.lower()

    if _is_footerish(t):
        return False
    if _is_question_title(t):
        return False

    if low in STOP_TITLES or low in BAD_TITLES:
        return False

    # "Intro…practice" etc.
    if low.startswith("intro"):
        return False

    # too short or too long titles are usually garbage
    if len(t) < 10:
        return False
    if len(t) > 75:
        return False

    # too numeric
    digits = sum(ch.isdigit() for ch in t)
    if digits >= 6:
        return False

    # must have at least 2 real words
    words = re.findall(r"[A-Za-z]{3,}", t)
    if len(words) < 2:
        return False

    return True

def _extract_definitions(text: str) -> List[Tuple[str, str]]:
    """
    Extract explicit definition-like statements.
    Pattern: 'X is ...' / 'X refers to ...' / 'X means ...'
    """
    t = _clean(text)
    out: List[Tuple[str, str]] = []

    # sentence-ish split
    candidates = re.split(r"(?<=[.!?])\s+", t)

    pat = re.compile(
        r"^\s*([A-Z][A-Za-z0-9/\-\s]{2,80})\s+"
        r"(is|are|refers to|means|can be defined as)\s+(.+)$"
    )

    for sent in candidates:
        sent = sent.strip()
        if len(sent) < 25 or len(sent) > 260:
            continue

        m = pat.match(sent)
        if not m:
            continue

        raw_title = _clean(m.group(1))
        verb = m.group(2)
        rest = _clean(m.group(3))

        title = _normalize_title(raw_title)

        if not _title_is_valid(title):
            continue

        definition = f"{title} {verb} {rest}"

        # Drop polluted definitions
        if _is_footerish(definition):
            continue

        out.append((title, definition))

    return out
def _extract_pattern_concepts(text: str) -> List[str]:
    """
    Extract concept-like titles from anywhere in the text using robust patterns.
    Great for PDFs where headings aren't clean.
    """
    t = _clean(text)
    found = []

    patterns = [
        r"\bThe Threat of [A-Z][A-Za-z\- ]{3,40}\b",
        r"\bStrategies to [A-Za-z ]{3,60}\b",
        r"\bPossible Responses to [A-Za-z\- ]{3,60}\b",
        r"\bSustaining Superior Performance\b",
        r"\bCompetitive Advantage\b",
    ]

    for pat in patterns:
        for m in re.finditer(pat, t):
            title = _normalize_title(m.group(0))
            # quick clean-ups
            title = re.sub(r"\s{2,}", " ", title).strip()
            if len(title) < 10 or len(title) > 70:
                continue
            if _is_footerish(title) or _is_question_title(title):
                continue
            found.append(title)

    # dedupe preserve order
    out = []
    seen = set()
    for x in found:
        k = x.lower()
        if k in seen:
            continue
        seen.add(k)
        out.append(x)
        # Dedupe "Threat of X" vs "The Threat of X"
    return out

def _extract_keypoints(text: str, max_points: int = 4) -> List[str]:
    """
    Extract bullet-like fragments as key points.
    """
    t = (text or "").replace("\x00", " ")
    parts = re.split(r"[•\n\r\t]| - | \u2022 ", t)

    cleaned: List[str] = []
    for p in parts:
        p = _clean(p)
        if 25 <= len(p) <= 170:
            if _is_footerish(p):
                continue
            if re.fullmatch(r"\d+", p):
                continue
            # skip question bullets
            if "?" in p:
                continue
            # remove slide-number/title pollution like "2 Sustaining Superior Performance"
            p = re.sub(r"^\d+\s+", "", p).strip()
            p = p.replace(" o ", " • ")
            
            # drop if it becomes too short after cleaning
            if len(p) < 25:
                continue
            cleaned.append(p)

    # dedupe preserving order
    seen = set()
    out: List[str] = []
    for p in cleaned:
        k = p.lower()
        if k in seen:
            continue
        seen.add(k)
        out.append(p)
        if len(out) >= max_points:
            break
    return out

# -----------------------
# Public API
# -----------------------

def build_concept_index(chunks: List[DocChunk], max_concepts: int = 120) -> List[Dict]:
    """
    Build concept_index used by SemanticValidator + Tutor engine.
    Output dict:
      {"id","title","definition","key_points","sources"}
    """
    concepts: Dict[str, Dict] = {}

    # 1) Definitions
    for ch in chunks:
        for title, definition in _extract_definitions(ch.text):
            cid = _slug(title)
            if not cid:
                continue
            entry = concepts.setdefault(cid, {
                "id": cid,
                "title": title,
                "definition": "",
                "key_points": [],
                "sources": set(),
            })
            if not entry["definition"] or len(definition) < len(entry["definition"]):
                entry["definition"] = definition
            entry["sources"].add(ch.source)

    # 2) Headings (first line of chunk)
    for ch in chunks:
        raw = (ch.text or "").strip()
        first = raw.split("\n", 1)[0] if "\n" in raw else raw[:120]
        first = _normalize_title(first)

        if not _title_is_valid(first):
            continue

        cid = _slug(first)
        if not cid:
            continue

        entry = concepts.setdefault(cid, {
            "id": cid,
            "title": first,
            "definition": "",
            "key_points": [],
            "sources": set(),
        })
        entry["sources"].add(ch.source)
    # 2.5) Pattern concepts (PDF-friendly)
    for ch in chunks:
        for title in _extract_pattern_concepts(ch.text):
            cid = _slug(title)
            if not cid:
                continue
            entry = concepts.setdefault(cid, {
                "id": cid,
                "title": title,
                "definition": "",
                "key_points": [],
                "sources": set(),
            })
            entry["sources"].add(ch.source)

    # 3) Attach key points safely
    for ch in chunks:
        kps = _extract_keypoints(ch.text)
        if not kps:
            continue

        raw = (ch.text or "").strip()
        first = raw.split("\n", 1)[0] if "\n" in raw else raw[:120]
        first = _normalize_title(first)

        candidate_id = None

        # A) Starts with a valid title that matches a concept
        if _title_is_valid(first):
            cid = _slug(first)
            if cid in concepts:
                candidate_id = cid

        # B) Otherwise, attach if the concept title appears in the chunk (long titles only)
        if candidate_id is None:
            low = (ch.text or "").lower()
            for cid, entry in concepts.items():
                t = entry["title"].lower()
                if len(t) >= 14 and t in low:
                    candidate_id = cid
                    break

        if candidate_id is None:
            continue

        entry = concepts[candidate_id]
        existing = {x.lower() for x in entry["key_points"]}
        for kp in kps:
            if kp.lower() not in existing:
                entry["key_points"].append(kp)
    # Dedupe "Threat of X" vs "The Threat of X" (keep "The Threat of ...")
    to_remove = []
    for cid, e in concepts.items():
        t = e["title"].strip()
        if t.lower().startswith("threat of "):
            alt = "The " + t
            alt_id = _slug(alt)
            if alt_id in concepts:
                to_remove.append(cid)
    for cid in to_remove:
        concepts.pop(cid, None)
    # Finalize
    out = list(concepts.values())
    for e in out:
        e["sources"] = sorted(list(e["sources"]))
        e["key_points"] = e["key_points"][:5]

    # Rank: definitions first, then keypoints; avoid weird titles
    def _rank(e):
        title = e["title"].strip()
        has_def = 1 if e["definition"] else 0
        kp = len(e["key_points"])
        title_len = len(title)
        return (-has_def, -kp, title_len)
    # Drop admin/metadata concepts (course logistics)
    filtered = []
    for e in out:
        low = e["title"].lower()
        if low.startswith("guest sessions"):
            continue
        if low.startswith("course material"):
            continue
        if low.startswith("course positioning"):
            continue
        filtered.append(e)
    out = filtered

    out.sort(key=_rank)
    return out[:max_concepts]